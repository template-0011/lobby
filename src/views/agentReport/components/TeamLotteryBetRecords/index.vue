...
async function fetchList() {
  ...
  loading.value = true;
  const res = await betRecordTeamHttp
    .normalBetRecordList(params)
    .catch(() => null);
  const { code, data } = res || {};
  if (code === "0") {
...
<template>
  ...
  <div>
    ...
    <el-select
      v-model="lotteryType"
      :placeholder="$t('web.i18nFront.label.allPlays')"
      style="width: 240px"
    >
      <el-option :label="$t('web.i18nFront.label.allPlays')" value="" />
    </el-select>
  </div>
    ...
    <el-table :data="tableData" border style="width: 100%" v-loading="loading" @row-click="handleRowClick">
      ...
      >
        <template #default="scope">
          <span>{{
            translateTextBy(
              lottoOwnerEnum.getDes(scope.row.lottoOwnerShip || "")
            )
          }}</span>
        </template>
      </el-table-column>
