<template>
  ...
  <template v-else>
    <el-dialog
      ...
      :title="modalConfig?.dialog?.title ? $t(modalConfig.dialog.title) : ''"
      @close="handleCloseModal"
    >
      <!-- 滚动 -->
      <el-scrollbar max-height="60vh">
        <div class="px-3 mb-5" v-if="modalConfig.alert">
          <el-alert v-bind="modalConfig.alert" />
        </div>
        <!-- 表单 -->
        <el-form
          ref="formRef"
  ...
  formRef.value?.validate((valid: boolean) => {
    if (valid) {
      ...
      props.modalConfig
        ...
        .then((success) => {
          if (success) {
            ElMessage.success(t("system.i18nSystem.hint.operationSuccess"));
            emit("submitClick");
            handleCloseModal();
          }
        })
