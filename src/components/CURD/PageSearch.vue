  ...
  <el-card shadow="never" class="mb-[10px]" v-show="visible">
    <el-form ref="queryFormRef" :model="queryParams" :inline="true">
      <template v-for="(item, index) in formItems" :key="item.prop">
        <template v-if="item.type === 'wallet'">
          <WalletSelector ref="WalletSelectorRef" v-model="queryParams" />
        </template>
        <template v-else>
          <el-form-item
          ...
          >
            ...
            <template v-if="item.type === 'input' || item.type === undefined">
              <el-input
                ...
                v-bind="item.attrs"
                :placeholder="
                  item?.attrs?.placeholder ? $t(item.attrs.placeholder) : ''
                "
            ...
            <template v-if="item.type === 'input-tag'">
              <div class="flex-center">
                ...
                <template v-if="inputTagMap[item.prop].inputVisible">
                  <el-input
                    :ref="(el: any) => (inputTagMap[item.prop].inputRef = el)"
            ...
            <template v-else-if="item.type === 'date-picker'">
              <el-date-picker
                ...
                v-bind="item.attrs"
                :start-placeholder="
                  item?.attrs ? $t(`${item?.attrs['start-placeholder']}`) : ''
                "
                :end-placeholder="
                  item?.attrs ? $t(`${item?.attrs['end-placeholder']}`) : ''
                "
              />
            </template>
          </el-form-item>
...
const WalletSelectorRef = ref<FormInstance>();
// 是否显示
...
function handleReset() {
  ...
  if (WalletSelectorRef.value?.resetFields) {
    WalletSelectorRef.value?.resetFields();
  }
